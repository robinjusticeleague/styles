...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (tx, rx) = channel();
    let mut watcher = new_debouncer(Duration::from_secs(1), None, move |res| {
...
fn process_html_file(
    ...
    if !added.is_empty() || !removed.is_empty() {
        ...
        timer = Instant::now();
        update_css_file(&new_classes)?;
        let write_css_duration = timer.elapsed();
        ...
        let timing_details = format!(
            ...
            format_duration(write_css_duration)
        );

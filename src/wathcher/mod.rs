...
pub fn start(state: Arc<Mutex<AppState>>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut debouncer: Debouncer<_, FileIdMap> = new_debouncer(Duration::from_millis(1), None, tx)?;
    debouncer.watcher().watch(Path::new("index.html"), RecursiveMode::NonRecursive)?;
